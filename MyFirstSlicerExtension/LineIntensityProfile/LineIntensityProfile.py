...
class LineIntensityProfileWidget(ScriptedLoadableModuleWidget):
  ...
  """
    ...
    def setup(self):
        ...
        self.inputSelector.setToolTip("Pick the first input.")
        parametersFormLayout.addRow("First Volume: ", self.inputSelector)

        # second input selector
        self.secondInputSelector = slicer.qMRMLNodeComboBox()
        self.secondInputSelector.nodeTypes = ["vtkMRMLScalarVolumeNode"]
        self.secondInputSelector.selectNodeUponCreation = True
        self.secondInputSelector.addEnabled = False
        self.secondInputSelector.removeEnabled = False
        self.secondInputSelector.noneEnabled = False
        self.secondInputSelector.showHidden = False
        self.secondInputSelector.showChildNodeTypes = False
        self.secondInputSelector.setMRMLScene(slicer.mrmlScene)
        self.secondInputSelector.setToolTip("Pick the second input.")
        parametersFormLayout.addRow("Second Volume: ", self.secondInputSelector)
    ...
    def onSelect(self):
        self.applyButton.enabled = self.inputSelector.currentNode() and self.secondInputSelector.currentNode() \
                                   and self.rulerSelector.currentNode()

    def onApplyButton(self):
        logic = LineIntensityProfileLogic()
        logic.run(self.inputSelector.currentNode(), self.secondInputSelector.currentNode(),
...
class LineIntensityProfileLogic(ScriptedLoadableModuleLogic):
  ...
  """
    ...
    def showChart(self, imageSamples, legendNames):
        ...
        for sample in imageSamples:
            ...
            for i in range(nDataPoints):
                array.SetComponent(i, 0, float(i))
                array.SetComponent(i, 1, sample.GetTuple1(i))
                array.SetComponent(i, 2, float(0))
        ...
        cvNodes = cvNodes.GetNextItemAsObject()

        chartNode = slicer.mrmlScene.AddNode(slicer.vtkMRMLChartNode())
        for pairs in zip(legendNames, doubleArrays):
            chartNode.AddArray(pairs[0], pairs[1].GetID())
        cvNodes.SetChartNodeID(chartNode.GetID())
        return
